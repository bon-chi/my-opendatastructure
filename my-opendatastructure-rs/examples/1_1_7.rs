...
fn main() -> io::Result<()> {
    ...
    let mut set: BinaryHeap<StringWithLen> = BinaryHeap::new();
    // let mut set: Vec<StringWithLen> = Vec::new();
    let mut writer = BufWriter::new(File::create("examples/.1_1_7.txt")?);

    for line in reader.lines() {
        if let Ok(line) = line {
            let _ = set.push(StringWithLen(line));
        }
    }
    for line in set.into_sorted_vec() {
        // set.sort();
        // for line in set {
